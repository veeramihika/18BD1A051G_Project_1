...
const dbName = 'hospital-inventory';
let db
...
app.post('/searchventilatorbyhospitalid',middleware.checkToken, (req, res) => {
    var hId = req.body.hId;
    console.log("search ventilator by hospital id");
    var data = db.collection("ventilators").find({ "hId": hId}).toArray().then(result => res.json(result));
});

//Search Ventilator by ventilator id
app.post('/searchventilatorbyventilatorid',middleware.checkToken, (req, res) => {
    var ventilatorId = req.body.ventilatorId;
    console.log("search ventilator by ventilator id");
    var data = db.collection("ventilators").find({ "ventilatorId": ventilatorId}).toArray().then(result => res.json(result));
});
